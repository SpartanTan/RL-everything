...
class EpsilonGreedy:
    ...
    def choose_action(self, action_space:gym.spaces.Discrete, state:int, qtable:np.ndarray):
...
def run_env():
    ...
    for run in range(params.n_runs):
        ...
        for episode in tqdm(episodes, desc=f"Run {run}/{params.n_runs} - Episodes", leave=False):
            state = env.reset(seed=params.seed)[0] # get the state, int
            step = 0
            done = False
            total_rewards = 0
...
def postprocess(episodes, params:Params, rewards:np.ndarray, steps:np.ndarray, map_size):
...
def qtable_directions_map(qtable, map_size):
    """
    - `qtable`: 25x4"""
    # reshape from 25x4 to 5x5, put the largest q-value in each cell
    qtable_val_max = qtable.max(axis=1).reshape(map_size, map_size)
    # 5x5 with index of the best action
...
if __name__ == "__main__":
    params = Params(
        ...
        state_size=None,
        proba_frozen=0.9,
        savefig_folder=Path("example-Frozenlake/figures"),
    )
    ...
    # qtables: n_runs x 25 x 4 (Assume 5x5 map)
    # qtable: 25 x 4
